...
export function EmployeeKPIPage() {
	...
	return (
		<div className="p-6 max-w-[1400px] mx-auto">
			...
			<div className="grid grid-cols-1 xl:grid-cols-[1.6fr_1.8fr] gap-6">
				{/* Левая колонка */}
				<div className="space-y-6">
					<div className="card p-4">
						<h2 className="font-semibold font-display mb-4" style={{ color: 'var(--text)' }}>Базовые показатели</h2>
												...
												<div className="p-3 bg-black/20 border-t border-white/5 space-y-3 text-xs">
													...
													{(!item.tasks || item.tasks.length === 0) ? (
														...
														<div className="flex flex-col gap-4">
															{/* Бар-диаграмма: план vs факт по задачам */}
															<div>
																<p className="text-[11px] font-semibold text-slate-300 mb-2">
																	Диаграмма: план / факт по задачам
																</p>
																{(() => {
																	// Строим данные: Plan=100%, Fact=фактический % от плана
																	const chartData = item.tasks.map(t => {
																		let totalPlan = 0
																		let totalFact = 0
																		Object.values(t.metrics || {}).forEach(m => {
																			totalPlan += Number(m.plan) || 0
																			totalFact += Number(m.fact) || 0
																		})
																		const factPct = totalPlan > 0
																			? Math.round((totalFact / totalPlan) * 100)
																			: (totalFact > 0 ? 100 : 0)
																		const label = (t.title || 'Без названия').length > 20
																			? (t.title || 'Без названия').slice(0, 20) + '…'
																			: (t.title || 'Без названия')
																		return { name: label, 'План (%)': 100, 'Факт (%)': factPct, _factPct: factPct }
																	})
																	// Максимум оси X: 100 или больше если факт > 100%
																	const maxX = Math.max(100, ...chartData.map(d => d['Факт (%)'])) + 10
																	return (
																		<ResponsiveContainer width="100%" height={Math.max(140, item.tasks.length * 52)}>
																			<BarChart
																				data={chartData}
																				layout="vertical"
																			...
																			>
																				...
																				{/* Серая полоса — план (всегда 100%) */}
																				<Bar dataKey="План (%)" fill="#334155" radius={[0, 3, 3, 0]} barSize={24} />
																				{/* Цветная полоса — факт в % от плана */}
																				<Bar dataKey="Факт (%)" radius={[0, 3, 3, 0]} barSize={24} >
																					{chartData.map((d, i) => {
																						const color = d._factPct >= 100 ? '#34d399' : d._factPct >= 70 ? '#fbbf24' : '#f87171'
																...
																<div className="flex flex-wrap items-center gap-4 mt-2 justify-end text-xs">
																	...
																	<span className="flex items-center gap-1">
																		<span className="w-3 h-3 rounded-sm bg-white border border-slate-500" />
																	...
																	<span className="flex items-center gap-1">
																		...
																		<span className="text-slate-400">70–99%</span>
																	</span>
															...
															{/* Текстовый список задач */}
															<div className="space-y-2 border-t border-white/5 pt-3">
																<p className="text-[11px] font-semibold text-slate-300 mb-1">Детализация по задачам:</p>
																{item.tasks.map((t, tIdx) => (
																	<div key={t.id || tIdx}
																		 className="bg-white/5 p-2 rounded-lg space-y-1">
																		<p className="font-medium text-slate-200">{t.title || 'Без названия'}</p>
